...
OpenLayers.Handler.Pinch = OpenLayers.Class(OpenLayers.Handler, {
    ...
    touchstart: function(evt) {
        if (this.hasTouchEventBug(evt)) { this.fixTouchEventBug(evt); }
        var propagate = true;
        this.pinching = false;
    ...
    // Detection of bug in iOS4 (possibly webkit)
    hasTouchEventBug: function(evt) {
        return evt.pageY === 0 && Math.floor(evt.clientY) > Math.floor(evt.pageY);
    },
    
    fixTouchEventBug: function(evt) {
        var scrollTop = document.body.scrollTop,
            scrollLeft = document.body.scrollLeft,
            pageY = evt.clientY,
            pageX = evt.clientX;

        evt.clientY = pageY - scrollTop;
        evt.clientX = pageX - scrollLeft;
        ...
        if (evt.touches) {
            for (var t, i=0, l=evt.touches.length; i<l; i++) {
                t = evt.touches[i];
                t.clientX -= scrollLeft;
                t.clientY -= scrollTop;
            }
        }
    },
